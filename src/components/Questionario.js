...
export default function Questionario() {
  ...
  function validarEtapa() {
    ...
    if (step === 3) return form.pj && form.disponibilidade_tempo;
    return true;
  }
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '32px 20px 80px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 2px 16px rgba(0,0,0,.07)', padding: 32 }}>
          ...
          {step === 1 && <>
            ...
            <Field label="Ano de conclusão" name="ano_conclusao" value={form.ano_conclusao} onChange={hf} placeholder="Ex: 2022" />
            <Field label="Registro no Conselho (CRBM) — número" name="crbm" value={form.crbm} onChange={hf} />
            <Field label="Tempo de experiência na área de estética" name="tempo_experiencia" value={form.tempo_experiencia} onChange={hf} placeholder="Ex: 3 anos" />
          ...
          {step === 3 && <>
            ...
            <Field label="Disponibilidade para início" name="disponibilidade_inicio" value={form.disponibilidade_inicio} onChange={hf} placeholder="Ex: Imediata, em 15 dias..." />
            <Field label="Valor de referência pretendido por procedimento (opcional)" name="valor_referencia" value={form.valor_referencia} onChange={hf} />
          </>}
          ...
          {step === 4 && <>
            ...
            <TextArea label="Descreva uma situação em que precisou lidar com um paciente insatisfeito ou com queixas específicas" name="situacao_paciente" value={form.situacao_paciente} onChange={hf} />
            <TextArea label="Como avalia sua facilidade para trabalhar em equipe e receber orientações e treinamentos?" name="trabalho_equipe" value={form.trabalho_equipe} onChange={hf} />
            <TextArea label="Tem perfil comercial para indicação de protocolos e fidelização de pacientes?" name="perfil_comercial" value={form.perfil_comercial} onChange={hf} />
