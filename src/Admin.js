...
const ADMIN_PASSWORD = 'Genthe2026';
...
async function loadCandidatos() {
  try {
    const { data, error } = await supabase.from('Candidatos_Suzuki').select('*').order('id', { ascending: false });
    if (error) { console.error(error); return []; }
    return data.map(r => ({ id: r.id, ts: r.created_at, ...r.data }));
    ...
    <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 2px 14px rgba(0,0,0,.07)', marginBottom: 18, overflow: 'hidden', border: '1px solid #E8E8E8' }}>
      ...
      {aberto && (
        <div style={{ padding: '22px 24px' }}>
          ...
          <p style={{ fontSize: 13, marginBottom: 4 }}>Instituição: {c.instituicao || '—'} — Conclusão: {c.anoConclusao || '—'}</p>
          <p style={{ fontSize: 13, marginBottom: 4 }}>Registro CRBM: {c.crbm || '—'} — Tempo de experiência: {c.tempoExp || '—'}</p>
          ...
          <p style={{ fontSize: 13, marginBottom: 4 }}>Sábados de manhã: {c.sabado || '—'} — Semana até o fim do expediente: {c.semana || '—'}</p>
          <p style={{ fontSize: 13, marginBottom: 4 }}>De acordo com modelo PJ: <strong>{c.pj || '—'}</strong></p>
          <p style={{ fontSize: 13, marginBottom: 4 }}>CNPJ: {c.cnpj || '—'} — Início: {c.disponibInicio || '—'} — Valor de referência: {c.valorReferencia || '—'}</p>

          {(c.motivacao || c.situacaoPaciente || c.trabalhoEquipe || c.perfilComercial) && <>
            ...
            {c.situacaoPaciente && <p style={{ fontSize: 13, marginBottom: 8 }}><strong>Situação com paciente:</strong> {c.situacaoPaciente}</p>}
            {c.trabalhoEquipe && <p style={{ fontSize: 13, marginBottom: 8 }}><strong>Trabalho em equipe:</strong> {c.trabalhoEquipe}</p>}
            {c.perfilComercial && <p style={{ fontSize: 13, marginBottom: 8 }}><strong>Perfil comercial:</strong> {c.perfilComercial}</p>}
          </>}
